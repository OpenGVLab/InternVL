...
for ((j=0; j<${#tasks[@]}; j++)); do
    ...
    if [ "${task}" == "vqa-chartqa-test" ]; then
        srun \
            ...
            --ntasks-per-node=1 \
            --job-name "wwy_eval" \
            -o "${LOG_DIR}/${model_name}/${task}_cot.log" \
            -e "${LOG_DIR}/${model_name}/${task}_cot.log" \
            --async \
        ...
        srun \
            ...
            -e "${LOG_DIR}/${model_name}/${task}_cot.log" \
            --async \
        sh evaluate.sh ${model_path} ${task} --dynamic --max-num 6 --cot "${ARGS[@]:1}"
    fi
done
