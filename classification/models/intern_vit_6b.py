...
class LayerScale(nn.Module):
    ...
    def forward(self, x):
        if self.force_fp32:
            output_type = x.dtype
            out = x.float().mul_(self.gamma.float(
            )) if self.inplace else x.float() * self.gamma.float()
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        x = self._naive_attn(
            x) if not self.use_flash_attn else self._flash_attn(x)
        return x
...
class InternViT6B(nn.Module):
    ...
    def __init__(self,
        ...
        use_flash_attn = [use_flash_attn] * depth if not isinstance(
            use_flash_attn, list) else use_flash_attn
        ...
        if cls_target == 'cls_patch_concat':
            ...
            self.head = nn.Linear(
                embed_dim *
                2, num_classes) if num_classes > 0 else nn.Identity()
    ...
    def init_weights(self, pretrained=None):
        ...
        def resize_pos_embed(pos_embed, H, W):
            cls = pos_embed[:, :1, :]
            pos_embed = pos_embed[:,
                                  1:, :].reshape(1, self.pretrain_size // 14,
                                                 self.pretrain_size // 14,
                                                 -1).permute(0, 3, 1, 2)
        ...
        if isinstance(pretrained, str):
            ...
            checkpoint['pos_embed'] = resize_pos_embed(
                pos_embed, self.img_size // self.patch_size,
